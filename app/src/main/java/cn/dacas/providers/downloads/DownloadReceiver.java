...
import cn.dacas.emmclient.mam.AppListFragment;
...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
        	...
        	//与DownloadInfo.java中的sendIntentIfRequested向对应
        	String fileName = intent.getStringExtra(Downloads._DATA);
			try{
				Intent toOpenApk = new Intent(Intent.ACTION_VIEW);
				toOpenApk.setDataAndType(Uri.fromFile(new File(fileName)),
						"application/vnd.android.package-archive");
				toOpenApk.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				context.startActivity(toOpenApk);
			}catch(Exception e){
				Toast.makeText(context.getApplicationContext(), "安装文件错误！", Toast.LENGTH_LONG).show();
				e.printStackTrace();
			}
			//TODO 刷新界面
			context.sendBroadcast(new Intent().setAction(AppListFragment.ACTION_REFRESH_APPSTORE));
