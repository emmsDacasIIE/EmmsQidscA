...
public class ExpandableAdapter extends ExpandableListItemAdapter<SortModel>
	...
	public ExpandableAdapter(Context mContext, List<SortModel> list, ContactDetailListener listener) {
		...
		mContactDetailListener = listener;
	}

	@Override
	public View getTitleView(final int position, View view, ViewGroup parent) {
...
//					.findViewById(R.id.contactIconImageView);
			...
			viewHolder.mUserRelativeLayout = (RelativeLayout) view
					.findViewById(R.id.user_main_layout);
		...
		viewHolder.mUserRelativeLayout.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				QDLog.i(TAG, "mUserRelativeLayout click:" + mContent.toString());
				mContactDetailListener.onDeliveData(mContent);
			}
		});
		return view;
	}
